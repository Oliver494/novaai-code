...
export function ProviderPanel({ projectPath, settings, onChange, onClose }: Props) {
  ...
  const groups = useMemo(() => ({
    local: settings.providers.filter((item) => providerMeta[item.provider].type === "local"),
    cloud: settings.providers.filter((item) => providerMeta[item.provider].type === "cloud"),
  }), [settings.providers]);
  ...
  return <div className="provider-overlay" role="dialog" aria-modal="true" aria-label={t("Proveedores de IA", "AI providers")}>
    <div className="provider-panel">
      ...
      <div className="provider-panel__body">
        <nav className="provider-list" aria-label="Lista de proveedores">
        ...
        <section className="provider-form">
          ...
          {(meta.requiresKey || selected === "custom") && <label>API key {selected === "custom" && <span className="field-hint">{t("Opcional: déjala vacía si tu servidor no usa autenticación.", "Optional: leave it empty if your server does not use authentication.")}</span>}<div className="key-row"><input type="password" value={key} onChange={(event) => setKey(event.target.value)} placeholder={draft.apiKeyConfigured ? "Guardada de forma segura" : "Pega tu clave"} autoComplete="new-password" /><span className={draft.apiKeyConfigured ? "key-state is-set" : "key-state"}><KeyRound size={13} />{draft.apiKeyConfigured ? "Configurada" : selected === "custom" ? t("Opcional", "Optional") : "Sin clave"}</span>{key.trim() && <button className="icon-button" onClick={() => void saveKey()} title="Guardar clave" disabled={saving}><Check size={15} /></button>}{draft.apiKeyConfigured && <button className="icon-button" onClick={() => void removeKey()} title="Eliminar clave"><Trash2 size={15} /></button>}</div></label>}
          ...
          <div className="timeout-grid"><label>Conexión<input type="number" min="1" max="60" value={draft.connectTimeoutSecs} onChange={(event) => update("connectTimeoutSecs", Number(event.target.value))} /><span>s</span></label><label>Inicio<input type="number" min="1" max="300" value={draft.firstResponseTimeoutSecs} onChange={(event) => update("firstResponseTimeoutSecs", Number(event.target.value))} /><span>s</span></label><label>Inactividad<input type="number" min="1" max="300" value={draft.inactivityTimeoutSecs} onChange={(event) => update("inactivityTimeoutSecs", Number(event.target.value))} /><span>s</span></label><label>Máximo<input type="number" min="10" max="3600" value={draft.maxResponseTimeoutSecs} onChange={(event) => update("maxResponseTimeoutSecs", Number(event.target.value))} /><span>s</span></label></div>
...
function ProviderGroup({ title, icon, items, selected, onSelect }: { title: string; icon: React.ReactNode; items: ProviderConfig[]; selected: ProviderId; onSelect: (id: ProviderId) => void }) {
  return <div className="provider-group"><span>{icon}{title}</span>{items.map((item) => <button key={item.provider} className={selected === item.provider ? "is-active" : ""} onClick={() => onSelect(item.provider)}><ProviderLogo provider={item.provider} size="medium" /><span>{providerMeta[item.provider].name}<small>{item.model || (providerMeta[item.provider].type === "local" ? "Local" : "Sin modelo")}</small></span>{item.apiKeyConfigured && <KeyRound size={12} />}</button>)}</div>;
}
